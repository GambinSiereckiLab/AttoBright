function outvector=fcscorrfit_1comptrip(infile,incolumn)
binning=1;
...
[fcsfit,gof,fitinfo]=fit(lags,corrcoeffs,ft,'StartPoint',[7,0.1,0.001,1000,10,0.1],'Lower',[3,0.00005,10^-6,1,10^-6,10^-9],'Upper',[20,1.5,0.01,100000,25,0.5]);
...
[fit2,gof,fitinfo] = fit(lags,corrcoeffs,ft,'StartPoint',[8.840,0.1,0.001,100,10,0.1],'Lower',[3,0.00005,10^-8,1,10^-6,10^-9],'Upper',[100,1.5,0.1,100000,10,0.1],'Exclude',outliers);

%Plot fit with points excluded
[filepath,name,ext] = fileparts(infile)
infile=name
mkdir(strcat(filepath,'./','figures\'));
outpathtot=strcat(filepath,'./','figures\',infile,'FCSonecomptriplet')
...
%outvecttot={'a (z0/w0)','G0','Ginf','Diffusion Time','Te','Tp','r.m.s.e.'};
%outvecttot=[outvecttot;outvector];
%outpath=strcat(infile2,'FCSonecomptriplet');
