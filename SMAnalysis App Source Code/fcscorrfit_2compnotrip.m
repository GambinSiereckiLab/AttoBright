function outvector=fcscorrfit_2compnotrip(infile,incolumn)
...
[fcsfit,gof,fitinfo]=fit(lags,corrcoeffs,ft,'StartPoint',[5,0.8,0.1,0.001,10,1000,10^-8,10^-8],'Lower',[3,0.01,0.00005,10^-6,1,1,10^-9,10^-9],'Upper',[20,0.99,1.5,0.01,1000,10000,10^-6,10^-6]);
...
[fit2,gof,fitinfo]=fit(lags,corrcoeffs,ft,'StartPoint',[8.840,0.8,0.1,0.001,1000,10000,10^-8,10^-8],'Lower',[2,0.01,0.00005,10^-9,10,1,10^-9,10^-9],'Upper',[100,0.99,1.5,0.01,10000,100000,10^-6,10^-6],'Exclude',outliers);

%Plot fit with points excluded
[filepath,name,ext] = fileparts(infile)
infile=name
mkdir(strcat(filepath,'./','figures\'));
outpathtot=strcat(filepath,'./','figures\',infile,'FCStwocompnotriplet')
...
outvector2=gof.rmse;
outvector=[outvector1(:,1),outvector1(:,2),outvector1(:,3),outvector1(:,4),outvector1(:,6),outvector1(:,5),outvector2(:,1)];%outvector=fit2;
%outvector=gof.rmse;

%outvector=num2cell(outvector);
%outvecttot={'a (z0/w0)', 'Fraction Component 1' 'G0','Ginf','Component 1 Diffusion Time','Component 2 Diffusion Time','r.m.s.e.'};
%outvecttot=[outvecttot;outvector];
%outpath=strcat(infile,'FCStwocompnotriplet');
